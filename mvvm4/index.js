...
const DomScanner = class extends Scanner{
	...
	scan(target, _ = type(target, HTMLElement)){
			const binder = new Binder; const f = el=>{
...
const Binder = class extends ViewModelListener{
	...
	viewmodelUpdated(updated){
			...
			updated.forEach(v=>{
					...
					const [vm, el] = items[v.subKey]; const processor = this.#processors[v.cat];
	...
	render(viewmodel, _ = type(viewmodel, ViewModel)){
			...
			this.#items.forEach(item=>{
				const vm = type(viewmodel[item.viewmodel], ViewModel); const el = item.el;
